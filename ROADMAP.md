...
源提交 `a630793` 的 Linux 快检 `35482650495` 和完整 Tier-1 运行
`35482705887` 已通过，G3L-01 至 G3L-12 全部关闭；验收矩阵限定的无阻尼 T3
线性动力范围提升为 `verified`。Kernel Contract 0.3 提供无装配预检、预算/取消、
结果证据和确定性重启。非零/时变位移约束与阻尼明确不在
