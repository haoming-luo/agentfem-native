...
ABI 1.4 为静态各向异性 P1 扩散、T3/T4 线弹性体积项生成确定性逐单元 COO 和
节点载荷，也能应用规范 CSR，并按原 COO 贡献顺序回填其数值。T3/T4 并行入口
使用显式线程数、静态连续分块和固定载荷归并；原串行入口保持兼容。任何非零状态
都使全部输出缓冲区无效，调用方不得使用部分结果。布局和兼容规则见
