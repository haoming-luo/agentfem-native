...
- 波传播增量：自主 T3 固定—自由纵向条带以局部高斯位移起振，在反射返回前用
  两个节点传感点测到 32/64 划分的传播速度误差为 2.8693%/1.3294%，最大相对
  能量偏移为 4.2231e-3/1.1763e-3。解析波速来自项目规格的平面应力本构与密度，
  不使用第三方有限元结果；边界反射与宽频色散继续保留为明确缺口。源提交
  `c1b3c9f` 的 Linux 快检 `35463705055` 通过 201 项测试和全部快速门禁。
- 反射/色散增量：自由端反射峰值保持同相，64→128 划分时到时误差从 0.8039%
  降至 0.0373%，峰值幅比从 0.9756 趋近 0.9973；`sigma=0.02` 窄脉冲的传播
  误差高于 `sigma=0.08` 宽脉冲，并随细化下降。全部真值来自项目数学规格，
  没有使用第三方 FEM 结果。源提交 `7e67cda` 的 Linux 快检 `35481948296`
  通过 203 项测试和全部快速门禁。
- 成熟度边界：该增量保持 `implemented`。有限矩阵已冻结为 G3L-01 至 G3L-12，
  其中 G3L-01 至 G3L-10 已有本地证据；版本化动力学 Kernel Contract 和最新
  候选三平台验收仍未关闭。阻尼、非零/时变位移约束、非线性、MPI 和 GPU 不在
  本次限定声明内；隐式初值/更强预条件器属于后续 P3 优化。

## Kernel Contract 0.3 线性动力本地候选 — 2026-09-20

- 任务来源：关闭 Gate 3 线性验收矩阵 G3L-11，让 AgentFEM 通过稳定 JSON 边界
  使用已经过科学增量验证的 T3 动力主干，而不依赖 Native Python 私有对象或
  当前 AF-IR 形状。
- 自主实现：0.3 请求由项目自有网格、材料、质量装配、稀疏算子、积分器、能量
  账本、约束和检查点语义构建；未读取、复制、翻译或改写第三方有限元源码。
- 架构证据：专用 T3 动力规划器在不装配质量/刚度矩阵的情况下估计稀疏存储、
  历史和求解工作区，预算通过后才执行装配与自由自由度稳定性复核。
- 产品证据：JSON Schema、可运行示例、0.1/0.2 兼容、预算、取消、稳定错误码、
  证据摘要和检查点逐位续算均有定向测试；全库 208 项测试本地通过。
...
- 晋级策略：本提交只改变成熟度元数据和证据说明；数值与平台实现仍为已验收的
  `a630793`，因此按 CI 治理只运行 Linux 快检，不重复完整 Tier-1。晋级提交
  `628349c` 的 Linux 复核 `35483046496` 已通过 208 项测试、格式/静态检查、
  可移植契约示例和独立性扫描。
