# Native P1/T3/T4 and sparse C ABI 1.2

Status: ABI 1.2 is implemented in the packaged C++20 accelerator. The Rust
comparison spike implements only the ABI 1.0 diffusion subset.

## Version and ownership

`afn_p1_abi_version()` returns `0x00010002`, encoded as major 1 and minor 2.
...
ABI 1.1 added `afn_t3_elasticity_assemble_cells`. Its points and cells use the
...
ABI 1.2 adds `afn_t4_elasticity_assemble_cells`. Points contain
`node_count * 3` values; cells contain four indices; constitutive data contains
`cell_count * 36` row-major values; body force contains three global values;
COO outputs contain `cell_count * 144` entries using twelve node-major vector
DOFs per cell; and load contains `node_count * 3` values.

ABI 1.2 also adds `afn_csr_spmv`. Canonical CSR inputs contain `row_count + 1`
signed 64-bit pointers, `nonzero_count` signed 64-bit column indices and binary64
values, plus `column_count` vector values. The caller provides `row_count`
output values. Empty matrices remain a readable-layer case; the ABI entry point
requires non-empty buffers.

The Rust spike participates only in diffusion-kernel language comparisons. It
does not advertise the ABI 1.1 or 1.2 additions; production C++ is the sole
shipping native implementation at this milestone.
...
T4 computes `volume * B^T D B` and the exact constant body-force load using the
strain and DOF order from `T4_LINEAR_ELASTICITY.md`. CSR SpMV computes each row
in increasing stored-column order. The readable implementations remain the
differential oracles for both additions.
