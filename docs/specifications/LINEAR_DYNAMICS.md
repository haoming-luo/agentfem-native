...
固定时间步的 Newmark 有效矩阵在一次执行段内保持不变。实现必须在循环外构造
`M + beta dt^2 K` 及其数值预条件器，随后每步只更新右端项并复用该资源；不得
使用进程全局缓存，也不得让重启隐式继承未序列化的求解器状态。
...
精确重启、受约束 T3 离散模态、固定—自由纵向波模态的空间频率收敛、反射前
局部脉冲传播速度、长时能量、反力恢复、外力功平衡，以及不安全显式时间步的
失败语义。固定—自由条带还必须证明自由端位移反射同相，以及窄脉冲相对宽脉冲
具有更明显、并随网格细化下降的离散色散。Kernel Contract 0.3 已接纳本规格
限定的 T3 动力范围；更强隐式预条件器属于 P3 优化，非零约束功在本次范围外。
源提交 `a630793` 已通过完整 Tier-1 `35482705887`，因此只有上述限定范围晋级为
`verified`；不得把该结论外推到整个 Gate 3 或非线性能力。
